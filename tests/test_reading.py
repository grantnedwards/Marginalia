...
async def test_plan_writes_checkpoints_and_reminder_fanout(db):
    ...
    assert await r.apply_plan(db, 1, cps, "America/Chicago", 34) == (8, 24)
...
async def test_reschedule_drops_pending_keeps_sent(db):
    await r.apply_plan(db, 1, schedule.plan(34, 8, **PLAN), "America/Chicago", 34)
    await db.run("UPDATE reminders SET status = 'sent' WHERE kind = 'unlock'")
    _, new = await r.apply_plan(db, 1, schedule.plan(34, 8, **PLAN), "America/Chicago", 34)
...
async def test_shorter_replan_sweeps_only_this_cohorts_surplus_weeks(db):
    ...
    await r.set_meeting(db, 1, date(2026, 4, 12), time(19, 0), "America/Chicago")
    await r.apply_plan(db, 1, schedule.plan(34, 8, **PLAN), "America/Chicago", 34)
    await r.apply_plan(db, 2, schedule.plan(34, 8, **PLAN), "America/Chicago", 34)

    async def create(cp):
        return 906

    await r.unlock(db, (await db.one("SELECT id FROM checkpoints WHERE cohort_id = 1"
                                     " AND idx = 6"))[0], create)
    await r.apply_plan(db, 1, schedule.plan(34, 4, **PLAN), "America/Chicago", 34)
...
async def test_next_checkpoint_breaks_tie_on_idx(db):
    ...
    assert (await r.next_checkpoint(db, 1, due - 1))["idx"] == 2
    assert await r.next_checkpoint(db, 1, 1 << 40) is None
...
async def test_unlock_is_idempotent(db):
    ...
    assert await r.unlock(db, cp_id, create) == 999 == await r.unlock(db, cp_id, create)
    assert calls == [1]
...
def test_illegal_auto_archive_duration_rejected():
    assert r.thread_kwargs("x")["type"].name == "public_thread"
    with pytest.raises(ValueError):
        r.thread_kwargs("x", 2880)


async def test_progress_accepts_a_lower_number(db):
    await r.set_progress(db, 1, U, 12, "chapter")
    await r.set_progress(db, 1, U, 3, "chapter")  # re-reading, or fixing a typo
    assert await r.progress_of(db, 1, U, "chapter") == 3  # latest wins, never clamped
    # The read side keeps BOTH reports, newest first: the lower one is history, not a fix.
    rows, page, pages = await r.history_page(db, 1, U, "chapter")
    assert [x["n"] for x in rows] == [3, 12] and (page, pages) == (1, 1)
    assert await r.mark_dnf(db, 1, U, "lost the thread") == (1, 1)
...
async def test_progress_history_pagination_boundaries(db, n, pages, last):
    for i in range(n):
        await r.set_progress(db, 1, U, i + 1, "chapter")
    assert (await r.history_page(db, 1, U, "chapter", 1))[1:] == (1, pages)
    assert len((await r.history_page(db, 1, U, "chapter", 99))[0]) == last  # 99 clamps
...
async def test_meeting_anchor_and_its_two_reminders_persist(db):
    ...
    await db.run("INSERT INTO cohorts (id, guild_id, channel_id, book_id, cycle_month, tz_id)"
                 " VALUES (2,?,3,1,'2026-04','America/Chicago')", G)
    await r.apply_plan(db, 2, schedule.plan(34, 8, **PLAN), "America/Chicago", 34)
    w, rem = await r.set_meeting(db, 1, date(2026, 4, 12), time(19, 0), "America/Chicago")
    assert rem == 2
    ...
    assert (await r.pace_of(db, 1, U, unix(w.instant)))[2] == "chapter"
...
async def test_no_plan_yields_no_unit_to_guess(db):
    ...
    assert await r.unit_of(db, 1) is None
    await r.apply_plan(db, 1, schedule.plan(400, 4, **{**PLAN, "by": "pages"}),
                       "America/Chicago", 34)
    assert await r.unit_of(db, 1) == "page"


async def test_pages_plan_reads_its_unit_past_the_meeting_anchor(db):
    pages = {**PLAN, "by": "pages"}
    w, _ = await r.set_meeting(db, 1, date(2026, 4, 12), time(19, 0), "America/Chicago")
    await r.apply_plan(db, 1, schedule.plan(400, 4, **pages), "America/Chicago", 34)
    ...
    assert await r.unit_of(db, 1) == "page"
    assert (await r.pace_of(db, 1, U, unix(w.instant)))[2] == "page"


async def test_replan_resyncs_a_thread_pinned_before_the_book_was_ingested(db):
    cps = schedule.plan(400, 4, **{**PLAN, "by": "pages"})
    await r.apply_plan(db, 1, cps, "America/Chicago", 0)  # no chapter_count: ceilings floor to 0

    async def create(cp):
        return 999

    cp_id = (await db.one("SELECT id FROM checkpoints WHERE idx = 1"))[0]
    await r.unlock(db, cp_id, create)
    ...
    await r.apply_plan(db, 1, cps, "America/Chicago", 34)
...
async def test_library_pagination_boundaries(db, n, pages, last):
    ...
    assert (await r.library_page(db, G, 1))[1:] == (1, pages)
    assert len((await r.library_page(db, G, 99))[0]) == last  # 99 clamps to the last page
...
async def test_member_views_stay_ephemeral(db, name):
    ...
    await r.set_progress(db, 1, U, 3, "chapter")
    cog, inter = await _cog(db), Rec()
