...
| `bot.py` + cogs | One `deliver` callback dispatches on all kinds; restart backoff escalates 5/10/20/40/60, caps, resets, and gives up at 6 (sum 135s, asserted `< 600` = `deploy/watchdog.sh` STALE). **All three cogs load against a real `Marginalia` with no token and no HTTP**, registering the real command count: 25 top-level, 27 walked, 26 runnable (MEASURED 2026-09-05, not counted by hand -- the figures here were stale twice before). `/cycle-open` refuses outside `#florilegium` before `create_role`, with a right-channel positive control. |
...
3. Guild-scoped sync lands and all 25 top-level commands appear instantly.
