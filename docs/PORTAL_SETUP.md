...
3. Put it where the container reads it. On unRAID: Docker tab -> **marginalia** ->
   Edit -> `DISCORD_TOKEN` (a masked field) -> Apply. Anywhere else: into
   `deploy/.env` as `DISCORD_TOKEN=<token, no quotes, no spaces>`.
4. If you used `.env`: `chmod 600 .env`
...
4. `chmod 600 .env`
5. Apply the change (unRAID: Docker tab -> marginalia -> Edit -> Apply; compose: `docker compose up -d`; otherwise `docker restart marginalia`).
